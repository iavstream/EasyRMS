...
int aos_get_gmt_str_time(char datestr[AOS_MAX_GMT_TIME_LEN])
{
    ...
    if ((s = aos_convert_to_gmt_time(datestr, g_s_gmt_format, &result))
        != AOSE_OK) {
        aos_error_log("aos_convert_to_GMT failure, code:%d %s.", s, apr_strerror(s, buf, sizeof(buf)));
        return AOSE_INTERNAL_ERROR;
    }
...
int aos_query_params_to_string(aos_pool_t *p, aos_table_t *query_params, aos_string_t *querystr)
{
    ...
    int len;
    char sep = '?';
    char ebuf[AOS_MAX_QUERY_ARG_LEN+1];
    char abuf[AOS_MAX_QUERY_ARG_LEN*2+128];
    int max_len;
    ...
    for (pos = 0; pos < tarr->nelts; ++pos) {
        if ((rs = aos_url_encode(ebuf, telts[pos].key, sizeof(ebuf)-1)) != AOSE_OK) {
        ...
        if (telts[pos].val != NULL && *telts[pos].val != '\0') {
            if ((rs = aos_url_encode(ebuf, telts[pos].val, sizeof(ebuf)-1)) != AOSE_OK) {
