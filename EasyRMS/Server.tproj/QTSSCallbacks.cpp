...
QTSS_Error QTSSCallbacks::Easy_StartHLSSession(const char* inSessionName, const char* inURL, UInt32 inTimeout, char* outURL)
{
	...
	packetParams.easyRecordOpenParams.inStreamName = (char*)inSessionName;
	packetParams.easyRecordOpenParams.inRTSPUrl = (char*)inURL;
	packetParams.easyRecordOpenParams.inTimeout = inTimeout;

	UInt32 fCurrentModule = 0;
	UInt32 numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kRecordOpenRole);
	for (; fCurrentModule < numModules; fCurrentModule++)
	{
		QTSSModule* theModule = QTSServerInterface::GetModule(QTSSModule::kRecordOpenRole, fCurrentModule);
		(void)theModule->CallDispatch(Easy_RecordOpen_Role, &packetParams);	
		return QTSS_NoErr;
	}
...
QTSS_Error QTSSCallbacks::Easy_StopHLSSession(const char* inSessionName)
{
	//¹Ø±ÕHLS Module
	QTSS_RoleParams packetParams;
	packetParams.easyRecordCloseParams.inStreamName = (char*)inSessionName;

	UInt32 fCurrentModule = 0;
	UInt32 numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kRecordCloseRole);
	for (; fCurrentModule < numModules; fCurrentModule++)
	{
		QTSSModule* theModule = QTSServerInterface::GetModule(QTSSModule::kRecordCloseRole, fCurrentModule);
		(void)theModule->CallDispatch(Easy_RecordClose_Role, &packetParams);
		return QTSS_NoErr;
	}
