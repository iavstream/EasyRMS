...
{
    extern char* optarg;//命令参数，例如执行：EasyRMS.exe -d -v
    int ch;
    
    char* theXMLFilePath = "./easyrms.xml";
   ...
   char* compileType = "Compile_Flags/_RELEASE;";
    ...
    while ((ch = getopt(argc,argv, "vdp:c:irsS:I")) != EOF) //opt: means requires option
    {
        switch(ch)
        {
            case 'v':
                ...
                qtss_printf("-i: Install the EasyRMS service\n");
                qtss_printf("-r: Remove the EasyRMS service\n");
                qtss_printf("-s: Start the EasyRMS service\n");
            ...
            case 'S':
                ...
                break;
            case 'i':
                qtss_printf("Installing the EasyRMS Server service...\n");
                ::InstallService("EasyRMS");
                qtss_printf("Starting the EasyRMS Server service...\n");
                ::RunAsService("EasyRMS");
                ::exit(0);
                break;
            case 'r':
                qtss_printf("Removing the EasyRMS Server service...\n");
                ::RemoveService("EasyRMS");
                ::exit(0);
            case 's':
                qtss_printf("Starting the EasyRMS Server service...\n");
                ::RunAsService("EasyRMS");
    ...
    {
		qtss_printf("EasyRMS Server Has Expired\n");
        ::exit(0);
    }
    ...
    qtss_printf("EasyRMS must either be started from the DOS Console\n");
    qtss_printf("using the -d command-line option, or using the Service Control Manager\n\n");
    qtss_printf("Waiting for the Service Control Manager to start EasyRMS...\n");
...
void InstallService(char* inServiceName)
{
    ...
    if (!theSCManager)
    {
        qtss_printf("Failed to install EasyRMS Service\n");
        return;
    }
    ...
    {
        ::CloseServiceHandle(theService);
        qtss_printf("Installed EasyRMS Service\n");
    }
    else
        qtss_printf("Failed to install EasyRMS Service\n");
...
void RemoveService(char *inServiceName)
{
    ...
    if (!theSCManager)
    {
        qtss_printf("Failed to remove EasyRMS Service\n");
        return;
    }
    ...
    if (theService != NULL)
    {
        ...
        ::CloseServiceHandle(theService);
        qtss_printf("Removed EasyRMS Service\n");
    }
    else
        qtss_printf("Failed to remove EasyRMS Service\n");
