...
#if __MacOSX__
StrPtrLen               QTSServerInterface::sServerNameStr("EasyRMS");
#else
StrPtrLen               QTSServerInterface::sServerNameStr("EasyRMS");
#endif
    ...
    fNumThreads(0)
{
	...
	qtss_sprintf(fDMSServiceID, "EasyRMS%s", "Config EasyRMS Uid");
...
{
    OSMutexLocker locker(fRecordSessionMap->GetMutex());
    for (OSRefHashTableIter theIter(fRecordSessionMap->GetHashTable()); !theIter.IsDone(); theIter.Next())
    {
		fRecordSessionMap->TryUnRegister(theIter.GetCurrent());
    }   
}
